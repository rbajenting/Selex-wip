...
function NKTableViewBindToDataBase(dbName, tableName, optionalQuery, autoCommit)
{
    if (autoCommit == null)
    {
        autoCommit = 'yes';
    }
    CallNKitAction("NKTableViewBindToDataBase?id="+this.id+"&name="+dbName+"&value="+tableName+"&query="+optionalQuery+"&autoCommit="+autoCommit);
}
...
{
	CallNKitAction("NKTableViewInsertRecord?id="+this.id+"&title="+encodeURIComponent(title)+"&subtitle="+encodeURIComponent(subtitle)+"&imageID="+image.id+"&section="+sectionNumber+"&rightImageID="+rightImage.id+"&callback="+callback);
}
...
function NKOpenDocument(name, source, x, y)
{
    CallNKitAction("NKOpenDocument?name="+encodeURIComponent(name)+"&source="+source+"&x="+x+"&y="+y);
}
...
function NKBanner()
{
    ...
    this.setLandscape = NKBannerSetLandscape;
    this.id = CallNKitAction("NKNextFreeID?sync=yes");
}

function NKBannerInit(x, y, orientation)
{
    if (orientation == 'no' || orientation == '0')
    {
        orientation = 'portrait';
    }
    else if (orientation == 'yes' || orientation == '1')
    {
        orientation = 'landscape';
    }
    
    CallNKitAction("NKBannerInit?id="+this.id+"&x="+x+"&y="+y+"&orientation="+orientation);
}
...
function NKBannerSetLandscape(isLandscape)
{
    CallNKitAction("NKBannerSetLandscape?id="+this.id+"&value="+isLandscape);
}
...
function NKFile()
{
    ...
    this.setSkipBackupBit = NKFileSetSkipBackupBit;
...
function NKFileSetSkipBackupBit(value) {
    CallNKitAction("NKFileSetBackupBit?id="+this.id+"&value="+value);
}
...
function NKSQLite()
{
    ...
    this.getLastErrorMessage=NKSQLiteGetLastErrorMessage;
    this.getLastErrorCode=NKSQLiteGetLastErrorCode;
}

function NKSQLiteGetLastErrorMessage() {
    return CallNKitAction("NKSQLiteGetLastErrorMessage?sync=yes");
}

function NKSQLiteGetLastErrorCode() {
    return CallNKitAction("NKSQLiteGetLastErrorCode?sync=yes");
}
...
function CallNKitAction(action)
{
	...
	request.open('POST','http://127.0.0.1:30001/?' + (Date.now() * Math.random()), false);
	request.send(pathArray[pathArray.length - 1] + '/' + action);
	if(request.status == 200) {
		return request.responseText;
	} else {
		alert('Failed to make remote API call');
	}
}
...
function NKVideoPlayer()
{
    ...
    this.seekToPosition=NKVideoPlayerSeek;
    this.setControlStyle=NKVideoPlayerSetControlStyle;
    this.pause=NKVideoPlayerPause;
    this.addLayer=NKVideoPLayerAddLayer;
    this.getPosition=NKVideoGetPosition;
	this.stop=NKVideoPlayerStop;
}

function NKVideoPLayerAddLayer(layer) {
    return CallNKitAction("NKVideoPLayerAddLayer?value="+layer);
}

function NKVideoGetPosition() {
    return CallNKitAction("NKVideoGetPosition?sync=yes");
}

function NKVideoPlayerPause() {
    return CallNKitAction("NKVideoPlayerPause");   
}

function NKVideoPlayerSetControlStyle(style) {
    return CallNKitAction("NKVideoPlayerSetControlStyle?value="+style);
}

function NKVideoPlayerSeek(seconds) {
    return CallNKitAction("NKVideoPlayerSeek?value="+seconds);
}
